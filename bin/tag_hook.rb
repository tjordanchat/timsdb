...
server.mount_proc '/' do |req, res|
  ...
  if /^refs\/tags\/RC_/ =~ ref
    ...
    email = `#{command}`
    bname = ref.split('/')[2]
    c1 = "gitlab project-branch create --project-id=%s --branch-name=%s --ref=%s" % [ pid, bname,ref ]
    ...
    protect = `#{c3}`
    body = "mail %s <<EOF
    A new branch '%s' was created for project %s with protection status = %s
EOF" % [ email, bname, pn, protect ]
    puts body
    `#{body}`
  end
end
